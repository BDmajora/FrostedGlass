...
static void reposition_shell_windows(struct fg_server *server) {
    /* Guard against early-boot calls before the server struct is fully ready */
    if (!server) return;

    /* Safely check top-level pointers without digging into unmapped surface structs */
    struct fg_toplevel *desktop = server->desktop;
    if (desktop && desktop->xdg_toplevel) {
        position_desktop(desktop);
    }

    struct fg_toplevel *taskbar = server->taskbar;
    if (taskbar && taskbar->xdg_toplevel) {
